    ...
    C: Has<XSpanIdString> + Clone + Send + Sync + 'static,
{
    ...
    async fn get_rates(
        &self,
        param_pair: models::Pair,
        param_period: Option<models::Period>,
        param_count: Option<i32>,
        param_base_datetime: Option<chrono::DateTime<chrono::Utc>>,
        context: &C,
        ...
        let mut uri = format!(
            "{}/api/v1/rates/{pair}",
            self.base_path,
            pair = utf8_percent_encode(&param_pair.to_string(), ID_ENCODE_SET)
        );

        // Query parameters
        let query_string = {
            let mut query_string = form_urlencoded::Serializer::new("".to_owned());
            if let Some(param_period) = param_period {
                query_string.append_pair("period", &param_period.to_string());
            }
            if let Some(param_count) = param_count {
                query_string.append_pair("count", &param_count.to_string());
            }
            if let Some(param_base_datetime) = param_base_datetime {
                query_string.append_pair("base_datetime", &param_base_datetime.to_string());
            }
            query_string.finish()
        };
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<models::GetRatesResponse>(body).map_err(|e| {
                    ApiError(format!("Response body did not match the schema: {}", e))
                })?;
                Ok(GetRatesResponse::Status200(body))
            }
            404 => {
                let body = response.into_body();
                let body = body
                ...
                Ok(GetRatesResponse::Status404(body))
            }
            code => {
