        ...
        C: Has<XSpanIdString>  + Send + Sync + 'static
    {
        ...
        match method {
            ...
            hyper::Method::GET if path.matched(paths::ID_RATES_PAIR) => {
                ...
                let path_params =
                    paths::REGEX_RATES_PAIR
                    .captures(path)
                    .unwrap_or_else(||
                        panic!("Path {} matched RE RATES_PAIR in set but failed match against \"{}\"", path, paths::REGEX_RATES_PAIR.as_str())
                    );
                ...
                let param_period = query_params.iter().filter(|e| e.0 == "period").map(|e| e.1.clone())
                    .next();
                let param_period = match param_period {
                    Some(param_period) => {
                        let param_period =
                            <models::Period as std::str::FromStr>::from_str
                                (&param_period);
                        match param_period {
                ...
                let param_base_datetime = query_params.iter().filter(|e| e.0 == "base_datetime").map(|e| e.1.clone())
                    .next();
